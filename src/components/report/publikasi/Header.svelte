<div class="slider-sub">
	<div class="bg-img">
		<img loading="lazy" src="/assets/images/banner/case-studies.png" alt="banner" />
	</div>
	<div class="container">
		<div class="heading-nav gap-4 mt-32">
			<a class="hover-underline caption1 text-white" href="/">Beranda</a><i
				class="ph ph-caret-double-right text-white"
			></i>
			<a class="hover-underline caption1 text-white" href="/laporan/publikasi">Laporan</a><i
				class="ph ph-caret-double-right text-white"
			></i>
			<div class="caption1 text-white">Laporan Publikasi</div>
		</div>
		<div class="text-nav">
			<div class="heading3 text-white">Laporan Publikasi</div>
			<div class="sub-heading mt-8 text-white fw-400">
				Transparansi kinerja dan pencapaian perusahaan untuk mendukung kepercayaan, profesionalitas,
				dan nilai berkelanjutan kami.
			</div>
		</div>
	</div>
</div>
